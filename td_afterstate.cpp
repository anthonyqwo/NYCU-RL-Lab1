...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (int a = 0; a < 4; ++a) {
			...
			candidate.set_value(candidate.reward() + estimate(candidate.after_state()));
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (size_t i = path.size() - 1; i > 0; --i) {
			...
			const board trained = current.after_state();
			// Re-select with CURRENT weights during the backward sweep, as specified.
			const state greedy = select_best_move(next.before_state());
			const float target = greedy.is_valid() ? greedy.value() : 0.0f;
	...
	void load(const std::string& path) {
		...
		std::string tag;
		std::getline(in, tag);
		if (tag != "RL2048-V1-td_afterstate") throw std::runtime_error("model format or TD variant mismatch");
		size_t count = 0;
...
int main(int argc, const char* argv[]) {
	try {
		...
		for (int i = 1; i < argc; ++i) {
			...
			if (arg == "--help") {
				info << "td_afterstate: sample-based TD(0) 2048 trainer\n"
		...
		if (!opt.eval && opt.save.empty()) opt.save = "td_afterstate.bin";
