...
## 訓練成果與里程碑比較 (10 萬局基準 vs. 100 萬局 vs. 120 萬局收斂)

本實驗除了完成課程規定的 100,000 局基礎訓練外，進一步延續訓練至 **1,200,000 局（包含 100 萬局里程碑）**。

![2048 TD Learning 1.2M 訓練收斂曲線](logs/training_curve_1200k.png)

數據摘要與里程碑比較如下：

### (1) 訓練歷程視窗指標（每階段前 10,000 局統計）

| 模型版本 | 局數階段 | 視窗平均分 | 歷史最高分 | 2048 勝率 | 4096 達成率 | 8192 達成率 | 累計訓練時間 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **TD-State** | 100k (基準) | 38,641 | 147,744 | 78.88% | 25.28% | 0.80% | ~19.3 分鐘 |
| | 500k (中期) | 66,253 | 240,032 | 89.10% | 63.50% | 19.81% | ~2.5 小時 |
| | **1,000k (1M)** | **79,373** | **270,572** | **91.10%** | **70.82%** | **33.26%** | **~7.0 小時** |
| | **1,200k (收斂)** | **82,942** | **278,904** | **91.76%** | **72.18%** | **36.78%** | **~8.9 小時** |
| | | | | | | | |
| **TD-Afterstate** | 100k (基準) | 40,838 | 132,012 | 76.88% | 36.34% | 0.95% | ~6.9 分鐘 |
| | 500k (中期) | 66,887 | 232,028 | 87.22% | 68.74% | 21.04% | ~48.3 分鐘 |
| | **1,000k (1M)** | **78,713** | **239,668** | **89.69%** | **75.93%** | **32.79%** | **~2.2 小時** |
| | **1,200k (收斂)** | **81,969** | **240,220** | **90.51%** | **77.13%** | **36.50%** | **~3.0 小時** |

### (2) 1,000 局獨立評估對比（Seed 2026，純推論）

| 模型版本 | 訓練規模 | 評估平均分 | 評估最高分 | 2048 勝率 | 4096 達成率 | 8192 達成率 | 權重模型檔路徑 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **TD-State (100k)** | 10 萬局 | 39,824.82 | 138,256 | 79.1% | 27.3% | 1.0% | `models/td_state_100k_20260924_202933.bin` |
| **TD-State (1.2M)** | **120 萬局** | **83,725.40** | **170,788** | **92.5%** | **72.1%** | **37.8%** | `models/td_state_1200k_20260924_232129.bin` |
| **TD-Afterstate (100k)** | 10 萬局 | 41,554.47 | 130,600 | 76.7% | 38.8% | 0.9% | `models/td_afterstate_100k_20260924_202933.bin` |
| **TD-Afterstate (1.2M)** | **120 萬局** | **84,512.50** | **168,480** | **90.3%** | **76.9%** | **36.4%** | `models/td_afterstate_1200k_20260924_232129.bin` |

* **Demo 推薦使用**：120 萬局模型已完整收斂，平均分數翻倍（>8.3 萬分），2048 勝率全面跨越 90%，且 8192 達成率達 36% 以上。
